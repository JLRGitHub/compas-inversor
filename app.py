# app.py
# -----------------------------------------------------------------------------
# El Compás del Inversor - v35.0 (Versión Web Final y Completa)
# -----------------------------------------------------------------------------
#
...
def calcular_puntuaciones_y_justificaciones(datos):
    ...
    if isinstance(per, (int, float)):
        if per < sector_bench['per_barato']: nota_valoracion += 6
        elif per < sector_bench['per_justo']: nota_valoracion += 4
        else: nota_valoracion += 2
    if isinstance(per, (int, float)) and isinstance(per_adelantado, (int, float)):
        if per_adelantado < per: nota_valoracion += 4 # Bonus por crecimiento esperado
    puntuaciones['valoracion'] = nota_valoracion
    if nota_valoracion >= 8: justificaciones['valoracion'] = "Valoración atractiva con crecimiento esperado."
